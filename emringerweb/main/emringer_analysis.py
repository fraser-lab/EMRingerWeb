...
def emringer_scan(args):
    # All processes that require reading the pickle. Involves reading out the angles and calculating the thresholds.
    # print "===== Running EMRinger: %s ====="
    chi = 1
    waves=[]
    averages=[]
    maxima=[]
    ...
    # for i in ringer_things:
    #   if chi in i._angles.keys() and i.resname in Residue_codes:
    #       print '{ "Residue Name": "%s",' % i.resname
    #       print '"Residue Number": %d,' % int(i.resid)
    #       print '"Chain ID": "%s",' % i.chain_id
    #       print '"Peak Angle": %d,' % i._angles[chi].peakchi 
    #       print '"Peak Map Value": %.5f,' % i._angles[chi].peakrho 
    #       print '"Map Values": %s' % [float("%.3f" % j) for j in i._angles[chi].densities]
    #       print "}"
    #       waves.append(i)
    #       total_counts[i.resname]+=1
    #       maxima.append(max(i._angles[chi].densities))
    ...
    result = {
        "Residues": residues,
        "Model Length": total_counts,
        "Error Residues": error_count,
    }

    # print '], "Model Length": {'
    # for key, value in total_counts.items():
    #   print '"%s": %d,' % (key, value)
    # print '"Total": %d' % sum(total_counts.values())
    # print "},"
    max_max = max(maxima)
    avg_avg = np.average(averages)
    thresholds = [avg_avg+i*(max_max-avg_avg)/20 for i in range(20)]
    # print '"Threshold List": %s,' % thresholds
    # print "===== Files Scanned ====="
    return waves, thresholds, result
...
def calc_ratio(count_list, args):
    ...
    total_angles=72 # 360 = 5 degrees per bin * 72 bins
    binsize=int(total_angles/6)
    first_loc=12 # 60 = 5 degrees per bin * 12 bins
...
def main(args):
    ...
    waves,thresholds,output_part=emringer_scan(args)
    output_dict.update(output_part)
    length = len(waves)
    peaks=OrderedDict()
            # calculate peaks and histogram

    output_dict['Thresholds'] = {}
    ...
    for threshold in thresholds:
        ...
        for i in Residue_codes:
            ...
            # print '"Rotamer Ratio": %.3f,' % rotamer_ratios_residues_n
            # print ""
    ...
    # easy_pickle.dump(file+'.output/adjusted_zscores.pkl', [10*i/math.sqrt(length) for i in zscores])
    # print 'Wrote ' + file+'.output/adjusted_zscores.pkl'
    ...
    for index, value in enumerate(zscores):
        if value == max(zscores):
            ...
            output_dict['Final Statistics'] = {
                ...
                "Total Residues Scanned": length,
                "EMRinger Score": (10*value/math.sqrt(length)),
            }
            break
